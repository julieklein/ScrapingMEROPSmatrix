...
public class MainSelecting {
    ...
    private List<DoubleBag> files = new ArrayList<DoubleBag>();
    ...
    public MainSelecting(String filename) throws IOException {
        ...
        try {
            ...
            for (String s : processedLinesList) {
                out.println(s);
            }
            String line;
    ...
    private MatrixEntry processSingleTable() {
        ...
        if (!(interestingP4 || interestingP3 || interestingP2 || interestingP1
                || interestingP1prime || interestingP2prime || interestingP3prime || interestingP4prime)) {
            return createEmptyMatrixElem();
        }
        ...
        for (int i = 0; i < 20; i++) {
            DMatrix[i][0] = validityP4 ? Matrix[i][0] != 0 ? Math.log(Matrix[i][0])
                    : -1.0 : 0.0;
            DMatrix[i][1] = validityP3 ? Matrix[i][1] != 0 ? Math.log(Matrix[i][1])
                    : -1.0 : 0.0;
            DMatrix[i][2] = validityP2 ? Matrix[i][2] != 0 ? Math.log(Matrix[i][2])
                    : -1.0 : 0.0;
            DMatrix[i][3] = validityP1 ? Matrix[i][3] != 0 ? Math.log(Matrix[i][3])
                    : -1.0 : 0.0;
            DMatrix[i][4] = validityP1prime ? Matrix[i][4] != 0 ? Math.log(Matrix[i][4])
                    : -1.0 : 0.0;
            DMatrix[i][5] = validityP2prime ? Matrix[i][5] != 0 ? Math.log(Matrix[i][5])
                    : -1.0 : 0.0;
            DMatrix[i][6] = validityP3prime ? Matrix[i][6] != 0 ? Math.log(Matrix[i][6])
                    : -1.0 : 0.0;
            DMatrix[i][7] = validityP4prime ? Matrix[i][7] != 0 ? Math.log(Matrix[i][7])
                    : -1.0 : 0.0;
        }
        ...
        for (int iP4 = 0; iP4 < 20; iP4++) {
            if (DMatrix[iP4][0] >= 0) {
                for (int iP3 = 0; iP3 < 20; iP3++) {
                    if (DMatrix[iP3][1] >= 0) {
                        DoubleBag b = new DoubleBag();
                        computeEntry(DMatrix[iP4][0], DMatrix[iP3][1], lndP4, lndP3,
                                lndP2, lndP1, lndP1prime, lndP2prime, lndP3prime,
                                lndP4prime, b);
        ...
        // XXX for this portion, I am not sure about the total
        // as far as I can tell, the cycles with findMin will always try to read
        // past the end of the files or of the arrays
        // setup files
        long total = 0;
        for (DoubleBag db : files) {
            total += db.size();
        }
        // notice that total is = 400*size of one file

        // setup sorter
        values = new double[files.size()];
        long position = 0;
        // this cycle moves the pointer in the files forward by 400 positions
        for (int i = 0; i < files.size(); i++) {
            values[i] = getNewValue(i, position);
        }

        // do the thing!!
        long current = 0;
        double[] Q = new double[3];
        // XXX here total is divided by 4, 8, 12 and 16 - notice that it still
        // means total is past the last value stored in any of the files
        for (int i = 1; i < 4; i++) {
            long limit = total / 4 * i;
            while (current++ != limit) {
                // nothing is being done with the value here, so position can
                // just be incremented
                // findMin(++position);
                ++position;
            }
            Q[i - 1] = findMin(++position);
        }
    ...
    private void computeEntry(double P4, double P3, double lndP4, double lndP3,
            double lndP2, double lndP1, double lndP1prime, double lndP2prime,
            double lndP3prime, double lndP4prime, DoubleBag lnprobaarray) {
        double P2;
        double P1;
        double P1prime;
        double P2prime;
    ...
    private double findMin(long position) {
        ...
        values[candi] = getNewValue(candi, position);
        return cand;
    }

    private double getNewValue(int i, long position) {
        if (position >= files.get(i).size() || position < 0) {
            return 1;
        }
        return files.get(i).get(position);
    }
